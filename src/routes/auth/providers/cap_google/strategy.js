...
function Strategy(options, verify) {
  ...
  this._verify = async (req, done) => {
    if (req.query && req.query.access_token) {
      return this.userProfile(req.query.access_token, done, req, verify);
    }

    done(null, false);
  }

  OAuth2Strategy.call(this, options, this._verify);
  this.name = 'cap_google';
...
/**
 * Authenticate request by delegating to Google using OAuth 2.0.
 *
...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.error_code && !req.query.error) {
    return this.error(new Error("Authorization error"));
  }
 ...
 * @access protected
 */
Strategy.prototype.userProfile = function (accessToken, done, req, verify) {
  var self = this;
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    verify(req, accessToken, null, profile, done);
  });
}
